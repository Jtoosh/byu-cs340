...
describe("m4 integrated test", () => {
  ...
  it("testCreateFormattedStringsWithAnswer", () => {
    // Done: Use ts-mockito when/thenCall to call a function when the mockService.processList is called
    when(mockService.processList(anything())).thenCall(() =>
      verify(mockService.processList(notNull())).called(),
    );
    // Done: Verify that the parameters passed to mockService.processList are not null. Do so in the thenCall function from the previous TODO.
